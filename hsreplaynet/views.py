...
class HomeView(TemplateView):
	...
	def get_context_data(self, **kwargs):
		...
		for card_class in CardClass:
			...
			context["player_classes"].append({
				...
				"standard_winrate": winrates[card_class]["standard_winrate"],
				"wild_winrate": winrates[card_class]["wild_winrate"],
			})
	...
	def get_class_data(self):
		...
		for c, values in data.items():
			...
			ret[class_key] = {"standard_winrate": 50, "wild_winrate": 50}
			for value in values:
				if value["game_type"] == BnetGameType.BGT_RANKED_STANDARD:
					winrate_key = "standard_winrate"
				elif value["game_type"] == BnetGameType.BGT_RANKED_WILD:
					winrate_key = "wild_winrate"
