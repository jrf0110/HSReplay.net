...
def do_all_redshift_etl_maintenance():
	...
	log.info("Beginning Full Maintenance Cycle at: %s" % cycle_start.isoformat())
	count = 0
	while did_work:
		count += 1
...
def do_redshift_etl_maintenance():
	...
	insert_candidate_track = RedshiftStagingTrack.objects.get_insert_candidate_track()
	if insert_candidate_track:
		if insert_candidate_track.is_able_to_insert:
			...
			min_wait = insert_candidate_track.quiescence_duration_minimum
			current_wait = insert_candidate_track.quiescence_duration_seconds
	...
	if active_track.track_should_close:
		...
		if not active_track.successor:
			...
			# So now we will spin wait for 60 seconds to allow it to finish setting up
			wait_for_condition(lambda: active_track.successor.is_ready_to_become_active, 60)
